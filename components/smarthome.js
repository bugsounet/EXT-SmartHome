...
class smarthome {
  constructor (config, cb = () => {}) {
    ...
    if (config.debug) log = (...args) => { console.log("[WEBSITE] [SMARTHOME]", ...args); };
  ...
  async init () {
    console.log("[WEBSITE] Loading smarthome...");
    ...
    let file = `${this.WebsiteModulePath}/smarthome.json`;
    fs.readFile(file, "utf8", (err, data) => {
      let content;
      if (!data) {
        console.error("[WEBSITE] [SMARTHOME] [HOMEGRAPH] smarthome.json: file not found!");
      ...
      try {
        ...
        console.error("[WEBSITE] [SMARTHOME] [HOMEGRAPH] smarthome.json: corrupt!");
      ...
      if (content.type && content.type === "service_account") {
        ...
        console.error("[WEBSITE] [SMARTHOME] [HOMEGRAPH] smarthome.json: bad format!");
    ...
    setTimeout(() => {
      if (this.website.website.initialized) {
        console.log("[WEBSITE] [SMARTHOME] Device Configuration...");
        this.initDevice();
      }
  ...
  createDevice () {
    ...
    this.SmartHome.device = {
      ...
      deviceInfo: {
        ...
        swVersion: require(`${this.WebsiteModulePath}/package.json`).version
      }
    };
  }
  ...
  initDevice () {
    ...
    if (this.SmartHome.homegraph) {
      ...
      console.log("[WEBSITE] [SMARTHOME] [DEVICE] HomeGraph is disabled.");
    }
  ...
  actions () {
    this.SmartHome.actions.onSync((body, headers) => {
      ...
      if (!user_id) {
        console.error("[WEBSITE] [SMARTHOME] [ACTIONS] [SYNC] Error: user_id not found!");
    ...
    this.SmartHome.actions.onExecute((body, headers) => {
      ...
      let user_id = this.check_token(headers);
      if (!user_id) {
        console.error("[WEBSITE] [SMARTHOME] [ACTIONS] [EXECUTE] Error: user_id not found!");
        return {}; // maybe return error ??
      }
      var result = {};
      result["payload"] = {};
    ...
    this.SmartHome.actions.onQuery((body, headers) => {
      ...
      let user_id = this.check_token(headers);
      if (!user_id) {
        console.error("[WEBSITE] [SMARTHOME] [ACTIONS] [QUERY] Error: user_id not found!");
        return {}; // maybe return error ??
      }
      var result = {};
      result["payload"] = {};
  ...
  check_token (headers) {
    ...
    if (!access_token) {
      console.error("[WEBSITE] [SMARTHOME] [TOKEN] No token found in headers");
      return null;
    }
    if (fs.existsSync(`${this.tokensDir}${access_token}`)) {
      ...
      console.error("[WEBSITE] [SMARTHOME] [TOKEN] Token not found in database", access_token);
      return null;
    }
  }
  ...
  delete_token (access_token) {
    if (fs.existsSync(`${this.tokensDir}${access_token}`)) {
      ...
      console.error("[WEBSITE] [SMARTHOME] [TOKEN] Delete Failed", access_token);
    }
  }
  ...
  async requestSync () {
    ...
    try {
      ...
      console.log("[WEBSITE] smarthome Ready!");
    } catch (e) {
      if (e.code) {
        console.error("[WEBSITE] [SMARTHOME] [HOMEGRAPH] [RequestSync] Error:", e.code, e.errors);
        this.send("Alert", `[requestSync] Error ${e.code} - ${e.errors[0].message} (${e.errors[0].reason})`);
      } else {
        console.error("[WEBSITE] [SMARTHOME] [HOMEGRAPH] [RequestSync]", e.toString());
  ...
  async queryGraph () {
    ...
    try {
      ...
      console.log("[WEBSITE] [SMARTHOME] [HOMEGRAPH] [QueryGraph]", e.code ? e.code : e, e.errors ? e.errors : "");
    }
  }
  ...
  async updateGraph () {
    ...
    if (!_.isEqual(current, old)) {
      ...
      try {
        ...
        console.error("[WEBSITE] [SMARTHOME] [HOMEGRAPH] [ReportState]", e.code ? e.code : e, e.errors ? e.errors : "");
      }
    }
  }
